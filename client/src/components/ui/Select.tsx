    ...
    return (
      <div className="w-full flex flex-col gap-1.5">
        ...
        <select
          ref={ref}
          id={selectId}
          className={`w-full rounded-xl border bg-white dark:bg-slate-800 px-4 py-2.5 text-sm outline-none transition-all duration-200 cursor-pointer appearance-none text-slate-900 dark:text-slate-100 bg-[url('data:image/svg+xml;charset=utf-8,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20viewBox%3D%220%200%2020%2020%2522%20fill%3D%22none%22%3E%3Cpath%20d%3D%22M7%209l3%203%203-3%22%20stroke%3D%22%2364748b%22%20stroke-width%3D%221.5%22%20stroke-linecap%3D%22round%22%20stroke-linejoin%3D%22round%22%2F%3E%3C%2Fsvg%3E')] bg-[length:1.25rem_1.25rem] bg-[right_0.875rem_center] bg-no-repeat pr-10
            ${
              error
                ? 'border-rose-500 text-rose-900 dark:text-rose-400 focus:border-rose-500 focus:ring-2 focus:ring-rose-500/20'
                : 'border-slate-200 dark:border-slate-600 focus:border-brand-primary focus:ring-2 focus:ring-brand-primary/20'
            }
            ${className}
          `}
          {...props}
